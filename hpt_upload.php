					...
					if($file['error'] == UPLOAD_ERR_OK) {
					  if(isAllowedExtension(strtolower($file['name']))) {
						// Uploading starts here
							...
							$uploadfile = getcwd ().$uploaddir . $newName;
							...
							if (move_uploaded_file($_FILES['hpt_files']['tmp_name'], $uploadfile)) {
								$url = get_bloginfo('url').'/wp-content/plugins/hungred-post-thumbnail/images/draft/'.$newName;
...
function sqlquery($url,$name,$loc)
{
	...
	if(file_exists($oripath) != false)
		unlink($oripath);
	$query = "REPLACE INTO $table(hpt_post, hpt_name, hpt_url, hpt_loc) VALUES('".$thisPost."', '".$name."', '".$url."', '".$loc."')";
